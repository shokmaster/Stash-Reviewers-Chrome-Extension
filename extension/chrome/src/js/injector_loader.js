...
storage.loadGroups(function(data) {
    storage.loadJunction(function(urlJunction) {
        if (data) {
            storage.loadHipChatUsername(function(username) {
                createInlineScript("var hipchatUsername = '" + (username || '') + "';");
            });

            createInlineScript("var jsonGroups = " + data + "; var chromeExtId='" + chrome.runtime.id + "'; stashIcon='" + chrome.extension.getURL('img/stash128.png') + "'; urlJunction='" + urlJunction + "';");
...
chrome.runtime.onMessage.addListener(function(message) {
    ...
    if (message && message.action === 'ActivitiesRetrieved') {
        var event = new CustomEvent('ActivitiesRetrieved', {
            'detail': {
